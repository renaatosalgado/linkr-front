...
export default function LikeHeart({ like, setLike, postId }) {
    ...
    useEffect(() => {
        api.totalLikes(postId, auth?.token).then(({ data }) => {
            setTotalLikes(data);
            if (data === 0) setDisabled(true);
            else setDisabled(false);
        });

        api.getTwoNames(postId, auth?.token).then(({ data }) => {
            mountNamesForTheTooltip(data);
        });

        api.checkLikeUser(postId, auth?.token).then(({ data }) => {
            setLike(data);
        });
    ...
    return (
        <LikeHeartWrapper>
            {like ? (
                <HeartIconSelect size="26" onClick={handleLike} />
            ) : (
                <HeartIcon size="26" onClick={handleLike} />
            )}
            <LikeText
                data-tip={nameTooltip}
                data-type="light"
                data-place="bottom"
